...
export const BottomTabNavigation = () => {
  ...
  const tabs = [
    ...
    { path: '#help', icon: HelpCircle, label: 'Help', isModal: true },
  ];
    ...
    <>
      <nav className="fixed bottom-0 left-0 right-0 bg-background border-t border-border safe-area-bottom z-40">
        <div className="flex items-center justify-around h-16 px-2">
          {tabs.map((tab) => {
            const isActive = !tab.isModal && location.pathname === tab.path;
            ...
            if (tab.isModal) {
              return (
                <button
                  ...
                  className="flex flex-col items-center justify-center min-w-0 flex-1 py-1 px-2 transition-colors duration-200 text-muted-foreground hover:text-foreground"
                >

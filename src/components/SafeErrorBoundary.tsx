...
export class SafeErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (isIOSSafari() && this.state.retryCount < 2) {
      const isConnectionError = error.message.includes('NetworkError') || error.message.includes('fetch') || error.message.includes('AbortError');
  ...
  public render() {
    if (this.state.hasError) {
      ...
      if (this.props.level === 'component') {
        return this.props.fallback || <div className="p-2 text-center text-muted-foreground">
      ...
      if (this.props.level === 'section') {
        return this.props.fallback || <Card className="m-4">
          ...
          <Card className="max-w-lg w-full">
            ...
            <CardContent className="space-y-4">
              <p className="text-muted-foreground">
                {isIOSSafari() ? <>Having trouble loading on iOS Safari. This can happen with poor connectivity or background app limits.</> : <>We're sorry, but something unexpected happened while loading {this.props.name}.</>}
              </p>
              
              {navigator.onLine ? <div className="flex items-center gap-2 text-success text-sm">
                  <Wifi className="h-4 w-4" />
                  Connected to internet
                </div> : <div className="flex items-center gap-2 text-destructive text-sm">
                  <WifiOff className="h-4 w-4" />
                  No internet connection
                </div>}
              
              <div className="flex gap-2">
                <Button onClick={this.handleRetry} className="flex-1 text-slate-50">
              ...
              {isIOSDevice() && <p className="text-xs text-muted-foreground">

                ...
                {petData.contacts.map((contact, index) => (
                  <div key={index} className="p-3 rounded-lg border shadow-sm">
                    <div className="flex items-center space-x-2">
                      <Phone className={`w-4 h-4 ${contact.contact_type === 'emergency' ? 'text-red-600' : 'text-brand-secondary'}`} />
                      <strong className={contact.contact_type === 'emergency' ? 'text-red-700' : 'text-brand-secondary'}>
                    ...
                    {/\d{3}/.test(contact.contact_phone) ? (
                      <div className="ml-6">
                        <a 
                          ...
                          className={`font-medium hover:opacity-80 ${contact.contact_type === 'emergency' ? 'text-red-800' : 'text-brand-secondary'}`}
                          aria-label={`Call ${contact.contact_name}`}
                        >
                          {contact.contact_phone}
                        </a>
                        <p className={`text-xs ${contact.contact_type === 'emergency' ? 'text-red-600' : 'text-brand-secondary/80'}`}>Tap to call</p>
                      </div>
                    ) : (
                      <p className={`ml-6 font-medium ${contact.contact_type === 'emergency' ? 'text-red-800' : 'text-brand-secondary'}`}>
                        {contact.contact_phone}
                      </p>
                    )}
                  </div>

...
export async function createPet(petData: {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // Add user_id to pet data - don't set petport_id as it will be auto-generated by trigger
      ...
      if (error) {
        ...
        // Check if it's a unique constraint violation on petport_id
        if (error.code === '23505' && error.message.includes('petport_id')) {
          console.log(`createPet: Pet Port ID collision detected on attempt ${attempt}, retrying...`);
          lastError = new Error(`Pet Port ID collision (attempt ${attempt}): ${error.message}`);
      ...
      console.log("createPet: Pet created successfully with ID:", data.id, "and Pet Port ID:", data.petport_id);
      return data.id;
    } catch (error) {
      ...
      if (!(error instanceof Error && error.message.includes('Pet Port ID collision'))) {
        throw lastError;
      }
    }
  }

...
export type PetWithDetails = {
  ...
  petpass_id: string | null; // New field from database
  bio: string | null;
  notes: string | null;
  state: string | null;
  county: string | null;
...
export function transformPetData(pet: PetWithDetails): any {
  return {
    ...
    petPassId: pet.pet_pass_id || pet.petpass_id, // Use either field (for backward compatibility)
    bio: pet.bio,
    notes: pet.notes,
    state: pet.state,
    county: pet.county,
...
export async function fetchUserPets(): Promise<any[]> {
  try {
    const { data: pets, error } = await supabase
      .from("pets")
      .select("id, name, breed, species, age, weight, microchip_id, pet_pass_id, petpass_id, bio, notes, state, county, created_at, updated_at, user_id")
...
export async function fetchPetDetails(petId: string): Promise<any | null> {
  try {
    // First fetch the pet with all required fields including the new petpass_id
...
export async function createPet(petData: {
  ...
  try {
    ...
    // Add user_id to pet data - don't set petpass_id as it will be auto-generated by trigger
    ...
    console.log("createPet: Pet created successfully with ID:", data.id, "and PetPass ID:", data.petpass_id);
    return data.id;
  } catch (error) {

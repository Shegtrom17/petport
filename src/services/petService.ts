...
export type PetWithDetails = {
  ...
  petport_id: string | null;  // Use petport_id to match database schema
  bio: string | null;
  notes: string | null;
  state: string | null;
  county: string | null;
...
export function transformPetData(pet: PetWithDetails): any {
  return {
    ...
    petPortId: pet.petport_id, // Map petport_id to petPortId for component compatibility
    petPassId: pet.petport_id, // Also map to petPassId to satisfy interface requirement
    bio: pet.bio,
    notes: pet.notes,
    state: pet.state,
    county: pet.county,
...
export async function fetchUserPets(): Promise<any[]> {
  try {
    // Use the actual column names from the database schema - use petport_id since that's what exists in DB
    ...
    return pets.map(pet => {
      ...
      const petWithDetails: PetWithDetails = {
        ...
        petport_id: pet.petport_id, // Use petport_id as it exists in DB
        bio: pet.bio,
        notes: pet.notes,
        state: pet.state,
        county: pet.county,
...
export async function fetchPetDetails(petId: string): Promise<any | null> {
  try {
    // First fetch the pet with all required fields - use petport_id since that's what exists in DB

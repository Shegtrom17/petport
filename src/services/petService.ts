...
export function transformPetData(pet: PetWithDetails): any {
  return {
    ...
    petPassId: pet.pet_pass_id, // Also map to petPassId to satisfy interface requirement
    bio: pet.bio,
    notes: pet.notes,
    state: pet.state,
    county: pet.county,
...
export async function createPet(petData: {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`createPet: Attempt ${attempt} - Starting pet creation process`);
      ...
      // Add user_id to pet data - don't set pet_pass_id as it will be auto-generated by trigger
      ...
      if (error) {
        ...
        // Check if it's a unique constraint violation on pet_pass_id
        if (error.code === '23505' && error.message.includes('pet_pass_id')) {
          console.log(`createPet: Pet Pass ID collision detected on attempt ${attempt}, retrying...`);
          lastError = new Error(`Pet Pass ID collision (attempt ${attempt}): ${error.message}`);
          
          // Add a small delay before retry to reduce collision probability
      ...
      console.log("createPet: Pet created successfully with ID:", data.id, "and Pet Pass ID:", data.pet_pass_id);
      return data.id;
    } catch (error) {
      console.error(`createPet: Attempt ${attempt} failed:`, error);
      lastError = error instanceof Error ? error : new Error("Unknown error occurred");
      
      // If it's not a constraint violation, don't retry
      if (!(error instanceof Error && error.message.includes('Pet Pass ID collision'))) {
        throw lastError;
      }
    }
  }
  ...
  console.error(`createPet: All ${MAX_RETRIES} attempts failed`);
  throw lastError || new Error("Failed to create pet after multiple attempts");
}

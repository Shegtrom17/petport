...
serve(async (req) => {
  ...
  try {
    ...
    // Use the high-quality OG image (1200x630 for optimal Facebook display)
    const ogImageUrl = `https://${req.headers.get('host') || 'petport.app'}/og/resume-og-hd.png`;
...
<body>
  <div class="container">
    <h1>${safe(name)}'s Professional Resume</h1>
    <p>${description}</p>
  ...
  ${redirect ? `<script>setTimeout(() => location.replace(${JSON.stringify(redirect)}), 1000);</script>` : ""}
    ...
    console.error("resume-share error", e);
